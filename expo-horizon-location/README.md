...
| `watchHeadingAsync` <br> `getHeadingAsync`                                                        | ✅ Supported    | ⚠️ Best-effort   | Uses device orientation sensors — magnetometer + accelerometer, or the fused rotation-vector sensor as a fallback. Works only when the headset exposes a usable sensor; otherwise rejects with `QuestFeatureUnavailableException`. Without a magnetometer, `trueHeading` may be unavailable (`-1`). |
